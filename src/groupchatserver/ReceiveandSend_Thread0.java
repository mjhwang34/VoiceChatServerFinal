 ...
 * @author user
 */
public class ReceiveandSend_Thread0 extends Thread {
    DatagramSocket socket = null;
    int curNum = 0;
    byte[] buffer=new byte[1024];
   public ReceiveandSend_Thread0(DatagramSocket socket, int curNum){
       this.socket = socket;
       this.curNum = curNum;
   }
    @Override
    public void run(){
        ...
        while(GroupChatServer.calling){
            try{
                ...
                DatagramPacket data1 = new DatagramPacket(buffer, 0, buffer.length, clientsinetaddress[1], clients[1]);
                socket1.send(data1);
               DatagramPacket data2 = new DatagramPacket(buffer, 0, buffer.length, clientsinetaddress[2], clients[2]);
                socket2.send(data2);
                 System.out.println("*"+clients[1]+" "+clients[2] + " "+clientsinetaddress[1]+clientsinetaddress[2]);
